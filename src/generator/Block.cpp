...
void Block::generate_Geometry(){
    ...
    while (runAnew){    
                        ...
                        if (Functions::check_PointIntersection(intersectionPoint, planes)){
                            ...
                            for (int l = 0; l != (int)vertices.size(); ++l){
                                if (checkEquality((intersectionPoint - vertices[l]).norm(), 0)){
                                    bool addVertId = true;
                                    for (auto& id: planesVerticesId[i]) if (id == l) addVertId = false;
                                    if (addVertId) planesVerticesId[i].push_back(l);
                                    addVertId = true;
                                    for (auto& id: planesVerticesId[j]) if (id == l) addVertId = false;
                                    if (addVertId) planesVerticesId[j].push_back(l);
                                    addVertId = true;
                                    for (auto& id: planesVerticesId[k]) if (id == l) addVertId = false;
                                    if (addVertId) planesVerticesId[k].push_back(l);
                                    verticeExist = true;
                                }
                            }
                            if (!verticeExist){
        ...
        for (int i = 0; i!=(int)planes.size(); ++i){
            ...
            if ((int)planesVerticesId[i].size() < 3) isThisPlaneOk = false;
            if ((int)planesVerticesId[i].size() >= 3){
                for (int j = 1; j != (int)planesVerticesId[i].size() - 1; ++j) {
                    for (int k = j + 1; k != (int)planesVerticesId[i].size(); ++k) {
                        ...
                        if (!checkEquality(area, 0)) isThisPlaneOk = true;
                    }
                }
            }
            if (!isThisPlaneOk) {
    ...
    for (int i = 0; i!= (int)polygons.size(); ++i){
        for (int j = 0; j!=(int)polygons[i].verticesId.size(); ++j){
            int auxId1 = j;
            int auxId2 = j+1;
            if (j == (int)polygons[i].verticesId.size()-1) auxId2 = 0;
            bool isNewEdge = true;
...
void Block::calculate_AlphaBeta(){
    ...
    if ((int)chordsLength.size() % 2 != 0) medianChord = chordsLength[((int)chordsLength.size() + 1)/2 - 1];
    if ((int)chordsLength.size() % 2 == 0) medianChord = 0.5*((chordsLength[(int)chordsLength.size()/2 - 1]) + (chordsLength[(int)chordsLength.size()/2]));
    ...
    for (auto& p: polygons) surfaceArea += p.area;
...
void Block::export_BlockVtk(std::string _fileName){
    ...
    int nTotalPoints = 0;
    for (auto p: polygons) nTotalPoints += (int)p.verticesId.size();
...
void Block::calculate_BoundingSphere(){
    ...
    for (int auxi = 0; auxi != 6; ++auxi){
        ...
        objective[1] = 0; 
        objective[2] = 0;
        if (auxi == 0) objective[0] = 1;
        if (auxi == 1) objective[0] = -1;
        if (auxi == 2) objective[1] = 1;
        if (auxi == 3) objective[1] = -1;
        if (auxi == 4) objective[2] = 1;
        if (auxi == 5) objective[2] = -1;
        ...
        if (auxi == 0 || auxi == 1) extremeCoordinates[auxi] = columnPrimal[0];
        if (auxi == 2 || auxi == 3) extremeCoordinates[auxi] = columnPrimal[1];
        if (auxi == 4 || auxi == 5) extremeCoordinates[auxi] = columnPrimal[2];
    }
...
void Block::calculate_InscribedSphere(){
    ...
    for (int k = 0; k < numberColumns; k++) {
        ...
        for (int i = 0; i < numberRows; i++) {
            rows[put] = i;
            if (k < numberColumns - 1) elements[put] = cZ(planes[i].unitVector[k]);
            if (k == numberColumns - 1) elements[put] = -1;
            put++;
        }
    }
    starts[numberColumns] = put;    
