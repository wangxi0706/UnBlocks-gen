...
Polygon::Polygon(Vector3r _unitVector, std::vector<int>& _verticesId, std::vector<Vector3r>& _vertices):
        ...
        verticesId(_verticesId)
        {
            ...
            ASSERT(checkEquality(unitVectorX.norm(), 1));
            ASSERT(checkEquality(unitVectorY.norm(), 1));
            Matrix3r Transf = Matrix3r::Zero();
            Transf(0,0) = unitVectorX[0]; Transf(0,1) = unitVectorX[1]; Transf(0,2) = unitVectorX[2];
            Transf(1,0) = unitVectorY[0]; Transf(1,1) = unitVectorY[1]; Transf(1,2) = unitVectorY[2];
            Transf(2,0) = _unitVector[0]; Transf(2,1) = _unitVector[1]; Transf(2,2) = _unitVector[2];
            ...
            for (int i = 0; i!= (int)verticesOf2DPolygon.size(); i++){
                ...
                if (i < (int)verticesOf2DPolygon.size()-1) Pnext = verticesOf2DPolygon[i+1];
                if (i == (int)verticesOf2DPolygon.size()-1) Pnext = verticesOf2DPolygon[0];
