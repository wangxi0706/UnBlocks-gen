...
class Plane{
public:
    Plane(Vector3r _center, Vector3r _unitVector): 
        center(_center), 
        unitVector(_unitVector)
        {d = _center.dot(_unitVector);};
    Plane(Vector3r _unitVector, double _d):
        unitVector(_unitVector),
        d(_d)
...
class Edge{
public:
    Edge(int _verticeIdA, int _verticeIdB):
        verticesIdA(_verticeIdA),
...
class Triangle{
public:
    Triangle(Vector3r _pointD, Vector3r _pointE, Vector3r _pointF):
        pointD(_pointD),
        pointE(_pointE),
        pointF(_pointF),
        unitVector(((_pointF-_pointD).cross(_pointF-_pointE))/((_pointF-_pointD).cross(_pointF-_pointE)).norm())
        {
            ...
            Vector3r maxCorner = Vector3r::Zero();
                if (_pointD[0] < minCorner[0]) minCorner[0] = _pointD[0];
                if (_pointE[0] < minCorner[0]) minCorner[0] = _pointE[0];
                if (_pointF[0] < minCorner[0]) minCorner[0] = _pointF[0];
                if (_pointD[1] < minCorner[1]) minCorner[1] = _pointD[1];
                if (_pointE[1] < minCorner[1]) minCorner[1] = _pointE[1];
                if (_pointF[1] < minCorner[1]) minCorner[1] = _pointF[1];
                if (_pointD[2] < minCorner[2]) minCorner[2] = _pointD[2];
                if (_pointE[2] < minCorner[2]) minCorner[2] = _pointE[2];
                if (_pointF[2] < minCorner[2]) minCorner[2] = _pointF[2];
                
                if (_pointD[0] > maxCorner[0]) maxCorner[0] = _pointD[0];
                if (_pointE[0] > maxCorner[0]) maxCorner[0] = _pointE[0];
                if (_pointF[0] > maxCorner[0]) maxCorner[0] = _pointF[0];
                if (_pointD[1] > maxCorner[1]) maxCorner[1] = _pointD[1];
                if (_pointE[1] > maxCorner[1]) maxCorner[1] = _pointE[1];
                if (_pointF[1] > maxCorner[1]) maxCorner[1] = _pointF[1];
                if (_pointD[2] > maxCorner[2]) maxCorner[2] = _pointD[2];
                if (_pointE[2] > maxCorner[2]) maxCorner[2] = _pointE[2];
                if (_pointF[2] > maxCorner[2]) maxCorner[2] = _pointF[2];
...
public:
    Line(){};
    Line(Vector3r _pointA, Vector3r _pointB):
        pointA(_pointA),
        pointB(_pointB)
        {};
