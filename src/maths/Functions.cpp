...
{
    Line line1T1(_triangle1.pointD, _triangle1.pointE);
    Line line1T2(_triangle2.pointD, _triangle2.pointE);
    Line line2T1(_triangle1.pointE, _triangle1.pointF);
    Line line2T2(_triangle2.pointE, _triangle2.pointF);
    Line line3T1(_triangle1.pointF, _triangle1.pointD);
    Line line3T2(_triangle2.pointF, _triangle2.pointD);

    Vector3r auxVec;
    std::vector<Vector3r> resultingPoints;
    if (lineTriangleIntersection(line1T1, _triangle2, auxVec))
        resultingPoints.push_back(auxVec);
    if (lineTriangleIntersection(line2T1, _triangle2, auxVec))
        resultingPoints.push_back(auxVec);
    if (lineTriangleIntersection(line3T1, _triangle2, auxVec))
        resultingPoints.push_back(auxVec);
    if (lineTriangleIntersection(line1T2, _triangle1, auxVec))
        resultingPoints.push_back(auxVec);
    if (lineTriangleIntersection(line2T2, _triangle1, auxVec))
        resultingPoints.push_back(auxVec);
    if (lineTriangleIntersection(line3T2, _triangle1, auxVec))
        resultingPoints.push_back(auxVec);
    if ((int)resultingPoints.size() < 2)
        return false;
    ...
    for (int i = 0; i != (int)resultingPoints.size() - 1; i++)
    {
        ...
        {
            if (checkEquality((resultingPoints[i] - resultingPoints[j]).norm(), 0))
                resultingPoints[j][0] = NAN;
        }
    }

    resultingPoints.erase(std::remove_if(std::begin(resultingPoints), std::end(resultingPoints), [](Vector3r &_p)
                                         { return (std::isnan(_p[0])); }),
                          std::end(resultingPoints));
    if ((int)resultingPoints.size() != 2 || checkEquality((resultingPoints[0] - resultingPoints[1]).norm(), 0))
        return false;
...
bool Functions::pointVolumeIntersection(const Vector3r &_point, const std::vector<Triangle> &_modelBoundary)
{
    ...
    {
        Vector3r auxVec;
        if (Functions::lineTriangleIntersection(LineXDir, Tri, auxVec))
            nIntersections1++;
    }

    if (nIntersections1 % 2 == 0)
        return false;
    return true;
}
...
double Functions::vectorsAngle(Vector3r _centerPoint, Vector3r _unitVec, Vector3r _point1, Vector3r _point2)
{
    ...
    Vector3r P1 = (_point1 - _centerPoint);
    P1.normalize();
    Vector3r P2 = (_point2 - _centerPoint);
    P2.normalize();
    ASSERT(checkEquality(P1.norm(), 1));
    ASSERT(checkEquality(P2.norm(), 1));
    double aux = P1.dot(P2);
    if (aux > 1 && checkEquality(aux, 1))
        aux = 1;
    if (aux < -1 && checkEquality(aux, -1))
        aux = -1;
    ASSERT(aux >= -1 && aux <= 1);
...
void Functions::eliminate_RedundantPlanes(std::vector<Plane> &_planes)
{
    for (int pId = 0; pId != (int)_planes.size(); ++pId)
    {
        ...
        if (!(checkEquality(abs(_planes[pId].unitVector.dot(solPoint) - _planes[pId].d), 0)))
            _planes[pId].d = NAN;
    }
    _planes.erase(std::remove_if(std::begin(_planes), std::end(_planes), [](Plane &_p)
