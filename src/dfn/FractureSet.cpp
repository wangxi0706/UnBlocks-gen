...
void FractureSet::add_BaecherFracture(double _meanDipDirection, double _meanDipAngle, double _fisherConstant, std::string _sizeDistribution, double _meanFractureSize, double _sigmaFractureSize){
    ...
    boost::mt19937 gen; 
    gen.seed(dfn->randomSeed); dfn->randomSeed++;
	...
	if (_fisherConstant >= 2){
		...
		if (_meanDipDirection >= 0 && _meanDipDirection < M_PI) kapa = _meanDipDirection + M_PI;
		if (_meanDipDirection >= M_PI && _meanDipDirection <= 2*M_PI) kapa = _meanDipDirection - M_PI;
        ...
        Quaternionr rotCorrectionUnitVec(AngleAxisr(randomAngle, poleMean)); rotCorrectionUnitVec.normalize();
	...
	//Add a fracture with random dip angle and random dip direction
		...
		if (randomDipDirection >= 0 && randomDipDirection < M_PI) kapa = randomDipDirection + M_PI;
		if (randomDipDirection >= M_PI && randomDipDirection <= 2*M_PI) kapa = randomDipDirection - M_PI;
    ...
    if (_sizeDistribution == "log") add_CircularFractureByUnitVec({randCoordinateX,randCoordinateY,randCoordinateZ},poleAllRotated,randomLogNormal());
    if (_sizeDistribution == "exp") add_CircularFractureByUnitVec({randCoordinateX,randCoordinateY,randCoordinateZ},poleAllRotated,randomExponential());
    if (_sizeDistribution == "det") add_CircularFractureByUnitVec({randCoordinateX,randCoordinateY,randCoordinateZ},poleAllRotated,_meanFractureSize);
}

void FractureSet::trim_FractureBorders(Fracture& _fracture){
    bool shouldTrim = false;
    for (auto& p: _fracture.borderPoints){
        if (p[0] < dfn->regionMinCorner[0] || p[0] > dfn->regionMaxCorner[0]) {shouldTrim = true; break;}
        if (p[1] < dfn->regionMinCorner[1] || p[1] > dfn->regionMaxCorner[1]) {shouldTrim = true; break;}
        if (p[2] < dfn->regionMinCorner[2] || p[2] > dfn->regionMaxCorner[2]) {shouldTrim = true; break;}
    }
    ...
    if (shouldTrim){
        ...
        for (int i = 0; i!= (int)_fracture.borderPoints.size(); i++){
            if (_fracture.borderPoints[i][0] < dfn->regionMinCorner[0] || _fracture.borderPoints[i][0] > dfn->regionMaxCorner[0]) _fracture.borderPoints[i][0] = NAN;
            if (_fracture.borderPoints[i][1] < dfn->regionMinCorner[1] || _fracture.borderPoints[i][1] > dfn->regionMaxCorner[1]) _fracture.borderPoints[i][0] = NAN;
            if (_fracture.borderPoints[i][2] < dfn->regionMinCorner[2] || _fracture.borderPoints[i][2] > dfn->regionMaxCorner[2]) _fracture.borderPoints[i][0] = NAN;
        }
        ...
        for (int i = 0; i!= (int)_fracture.borderPoints.size()-1; i++){
            for (int j = i+1; j!= (int)_fracture.borderPoints.size(); j++){
                if (checkEquality((_fracture.borderPoints[i] - _fracture.borderPoints[j]).norm(), 0)) _fracture.borderPoints[j][0] = NAN;
            }
        }
        _fracture.borderPoints.erase(std::remove_if(std::begin(_fracture.borderPoints), std::end(_fracture.borderPoints), [](Vector3r& _p){return (std::isnan(_p[0]));}), std::end(_fracture.borderPoints));
        ...
        for (int i = 0; i!= (int)_fracture.borderPoints.size(); ++i){
        int j = i+1;
        if (j == (int)_fracture.borderPoints.size()) j = 0;
...
void FractureSet::add_CircularFracture(PyList _center, double _dipDirection, double _dipAngle, double _radius){
	...
	if (dipDirection >= 0 && dipDirection < M_PI) kapa = dipDirection + M_PI;
	if (dipDirection >= M_PI && dipDirection <= 2*M_PI) kapa = dipDirection - M_PI;
	...
	for (int i = 0; i!= dfn->nFracBorderPoints; i++){
        ...
        Quaternionr q(AngleAxisr(rotAngle, unitVector)); q.normalize();
...
void FractureSet::add_CircularFractureByUnitVec(Vector3r _center, Vector3r _unitVector, double _radius){
	...
	for (int i = 0; i!= dfn->nFracBorderPoints; i++){
        ...
        Quaternionr q(AngleAxisr(rotAngle, _unitVector)); q.normalize();
